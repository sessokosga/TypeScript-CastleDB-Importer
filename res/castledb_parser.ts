...
const cdb: CastleDBJson = JSON.parse(rawData);

// --- HELPER FUNCTIONS ---

// Convert "tasks" -> "Task", "tasks@need" -> "TasksNeed"
function getClassName(sheetName: string): string {
...
function getDefaultValue(typeStr: string): string {
  const typeId = parseInt(typeStr.split(":")[0]);
  switch (typeId) {
    case 1:
      return '""';
 ...
 */\n\n`;
  ...
  const enumsGenerated: Set<string> = new Set();
  const classDefinitions: string[] = [];
  const dataLoadingLines: string[] = [];
  const sheetVariables: string[] = [];
  ...
  cdb.sheets.forEach((sheet) => {
    ...
    if (!isCustomPropSheet) {
      ...
      sheetVariables.push(
        `    public static ${listName}: ${className}[] = [];`,
      );
      sheetVariables.push(
        `    public static get${className}(id: string): ${className} | undefined { return this.${listName}.find(i => (i as any).id === id); }`,
      );
      ...
      sheet.lines.forEach((line) => {
        ...
        dataLoadingLines.push(
          `        GameData.${listName}.push(new ${className}(${lineJson}));`,
        );
      });
    }
  });
  ...
  console.log(
    `Successfully generated ${OUTPUT_FILE} with ${sheetVariables.length / 2} sheets and ${dataLoadingLines.length} entries.`,
  );
}
